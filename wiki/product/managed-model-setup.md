...
**Planned behavior, not an available feature.** This document records the requested direction as of 2026-09-20. The SDK still accepts locally supplied model packs; runtime downloads and the APIs described conceptually here have not been implemented.
...
The initial candidate is English. Exact weights, download size, memory guidance, redistribution rights and physical-device qualification are unresolved. Hebrew and other additional languages are not established. Every eventual catalog entry must explain language, purpose, download/installed size and engine compatibility; performance comparisons need measured evidence.
