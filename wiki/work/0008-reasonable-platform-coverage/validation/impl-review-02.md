...
Every frozen criterion AC-001 through AC-012 is met on this tree, including the prior AC-007 gap: denied macOS authorization is refused before `startAudio`, and unit tests now assert that denial mapping and `AgentErrorCode.permissionDenied`.
...
Full trailing result: 73 tests passed, exit 0. Suite included `test/platform_refusal_test.dart` and `test/permission_denied_test.dart`.

Standalone re-run of the AC-007 Dart case:

```
00:00 +0: denied start returns permissionDenied and does not start capture
00:00 +1: All tests passed!
PERMISSION_TEST_EXIT=0
```

### 5. `c++ -std=c++17 -I native/src native/tests/mic_permission_test.cpp -o /tmp/flva-mic-permission-review && /tmp/flva-mic-permission-review`

The compile and run produced no stdout or stderr. Exit 0.

```
MIC_PERMISSION_TEST_EXIT=0
```

### 6. `rg permissionDenied test example/test`

```
test/permission_denied_test.dart:    'denied start returns permissionDenied and does not start capture',
test/permission_denied_test.dart:            AgentErrorCode.permissionDenied,
test/permission_denied_test.dart:      code: 'permissionDenied',
RG_EXIT=0
```

### Additional checks required by the frozen criteria
...
`flutter build macos --debug` (cwd `example`):
...
Warnings were documentation comments in vendored `c-api.h` (`parameter 'p' not found`). The plugin still lacks Swift Package Manager support for macOS; extra SPM is outside this item. Exit 0.

This checkout's `macos/libs` listing: `libsherpa-onnx-c-api.dylib`, `libonnxruntime.1.17.1.dylib`, symlink `libonnxruntime.dylib`. Windows and Linux `libs/` folders on this macOS checkout contain only `.gitkeep`; a Windows or Linux Flutter desktop build is explicitly not required.

Independent source checks (not author claims):

- `rg permissionDenied` over `test/` and `example/test/` matches only `test/permission_denied_test.dart`.
- macOS start returns `permissionDenied` on `FLVA_MIC_DENY` and on `granted=0` before the control-queue dispatch that can reach `startAudio`.
...
| AC-005 | pass | `macos/Classes/FlutterLocalVoiceAgentPlugin.mm:208`, `macos/Classes/FlutterLocalVoiceAgentPlugin.mm:222` | yes |
| AC-006 | pass | `flutter build macos --debug` cwd `example`, exit 0 | yes |
| AC-007 | pass | `macos/Classes/FlutterLocalVoiceAgentPlugin.mm:125`, `native/tests/mic_permission_test.cpp:8`, `test/permission_denied_test.dart:11` | yes |
...
| AC-011 | pass | `macos/Classes/FlutterLocalVoiceAgentPlugin.mm:210`, `windows/flutter_local_voice_agent_plugin.cpp:819`, `linux/flutter_local_voice_agent_plugin.cc:804` | yes |
| AC-012 | pass | `macos/flutter_local_voice_agent.podspec:36`, `windows/CMakeLists.txt:61`, `linux/CMakeLists.txt:76` | yes |

AC-001: `_refuseUnsupportedHost` runs before `native.create`. `kIsWeb` and a closed `defaultTargetPlatform` switch refuse web and fuchsia. `test/platform_refusal_test.dart` injects `TargetPlatform.fuchsia`, expects `AgentErrorCode.unsupportedProfile`, and asserts `_FakePlatform.created` stays false. A supported-macOS control reaches create. Flutter web implementation is explicitly not required; the specified injected-target test exists and passed in command 4.
...
AC-003: Digest compare happens before `tar.extractall` and before any desktop `_copy_library`. The provision self-test mismatch cases for macos, windows, and linux raised `ValueError: Runtime archive SHA-256 mismatch`, CLI exit 1, destination listing `['keep-me.txt']`, and zero sherpa-named files.

AC-004: `DESKTOP_INSTALL` names the official v1.12.14 shared-library files. The matching-digest fixtures installed those names for all three desktop keys. This checkout already has the macOS expected dylibs under `macos/libs`. The Python test remaps the pinned digest onto a tiny fixture rather than re-downloading official bytes; the criterion allows a local matching archive or a recorded provisioned checkout. A successful exit with missing expected names would fail that test.
...
AC-006: The macOS example debug build exited 0 and produced `build/macos/Build/Products/Debug/flutter_local_voice_agent_example.app`. A non-zero build would fail this row. Windows and Linux Flutter desktop builds were not required.

AC-007: Denied or restricted `AVAuthorizationStatus` maps through `flva_mic_start_decision` to `FLVA_MIC_DENY`, which returns `FlutterError` code `permissionDenied` on the platform thread and returns before the control-queue dispatch that can reach `startAudio` (the only `AVAudioEngine` allocation). `AVAuthorizationStatusNotDetermined` plus `granted=0` does the same. `native/tests/mic_permission_test.cpp` asserts statuses 1 and 2, plus out-of-range values, return `FLVA_MIC_DENY` and not `FLVA_MIC_ALLOW`; that binary was compiled and run in command 5, exit 0. `test/permission_denied_test.dart` asserts `agent.start()` throws `AgentErrorCode.permissionDenied` and that lifecycle is not `running`. Command 6 is no longer empty. A device deny-prompt run was not performed; physical microphone-to-speaker qualification is explicitly not required, and that absence is not a defect.
...
- Previous round: `wiki/work/0008-reasonable-platform-coverage/validation/impl-review-01.md`
- Recurring findings: none
- Oscillating: no

Previous F-001 (no unit or bridge test that denied access returns `permissionDenied` and does not start capture) did not recur. This tree has `native/tests/mic_permission_test.cpp` and `test/permission_denied_test.dart`; command 5 and the permissionDenied Dart case both exited 0, and `rg permissionDenied test example/test` is no longer empty. Previous F-002 (index still describing 0008 as not yet implemented) is also not present on the current `wiki/INDEX.md:170` line.
